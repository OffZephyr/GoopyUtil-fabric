...
public class COMPRemoteScreen extends COMPBaseAppScreen {
    List<GoopyUtilEntity> entities = new ArrayList<>();
    public Identifier LIST = Identifier.of(GoopyUtil.MOD_ID, "textures/gui/computer/window_smallbar.png");
    public Identifier LIST_OFF = Identifier.of(GoopyUtil.MOD_ID, "textures/gui/computer/window_smallbar_off.png");
    GoopyUtilEntity control = null;
    ...
    public boolean mouseReleased(double mouseX, double mouseY, int button) {
        if(control == null) {
            if(selected > -1 && isOnButton(mouseX, mouseY, this.width / 2 + 248 / 2 - 2 - 32, this.height / 2 - 248 / 2 + 2, 16, 16)) {
                ...
                ((ISetCameraThirdPerson)MinecraftClient.getInstance().gameRenderer.getCamera()).setThirsPerson(true);
            }
        }
    ...
    public void resetEntity() {
        ...
        updateAnimatronics(MinecraftClient.getInstance().world);
        ((ISetCameraThirdPerson)MinecraftClient.getInstance().gameRenderer.getCamera()).setThirsPerson(false);
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        if(control == null) {
            ...
            for(int i = 0; i < entities.size(); i++) {
                GoopyUtilEntity entity = entities.get(i);
                ...
                if(y >= topCornerY && y + 18 <= topCornerY + appAvailableSizeY) {
                    ...
                    Identifier signal = Identifier.of(GoopyUtil.MOD_ID, "textures/gui/computer/signal_" + strength + ".png");
    ...
    void updateAnimatronics(World world) {
        ...
        List<GoopyUtilEntity> list = world.getNonSpectatingEntities(GoopyUtilEntity.class, box);
        List<GoopyUtilEntity> finalList = new ArrayList<>();
        for(GoopyUtilEntity entity : list) {
    ...
    public GoopyUtilEntity getControl() {
        return this.control;
    }
}
