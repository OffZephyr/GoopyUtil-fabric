...
public class EntitySkinScreen extends GoopyScreen{
    Identifier background = Identifier.of(GoopyUtil.MOD_ID, "textures/gui/skins/quad_bg.png");
    float bg_scroll_x = 0;
    float bg_scroll_y = 0;
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        List<EntitySkin> skinList = manager.getSkins(entity.getType());
        int minY = 0;
        ...
        for(int i = 0; i < skinCount; i++) {
            ...
            EntitySkin skin = i >= skinList.size() ? skinList.get(skinList.size() - 1) : skinList.get(i);
            if(isOnButton(mouseX, mouseY, x1, y1, x2 - x1, y2 - y1)){
                ((IEntityDataSaver)entity).getPersistentData().putString("Reskin", skin.getName());
            }
        }
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        if(manager.getSkin(entity.getType(), skin) == null) skin = manager.getSkins(entity.getType()).get(0).name();
        ...
        Text text = Text.translatable("entity.goopyutil.skin_screen.confirm");
    ...
    void renderIcons(DrawContext context, int mouseX, int mouseY, float delta){
        ...
        for(int i = 0; i < skinCount; i++) {
            ...
            float color = Objects.equals(skin.getName(), ((IEntityDataSaver) entity).getPersistentData().getString("Reskin")) ? 1 : 0.6f;

            drawRecolorableTexture(context, skin.getIcon(), x1, y1, 0, x2 - x1, y2 - y1, 0, 0, x2 - x1, y2 - y1, 1, 1, 1, 1);
            drawRecolorableTexture(context, skin.getIconOutline(), x1 - 1, y1 - 1, 0, x2 - x1 + 2, y2 - y1 + 2, 0, 0, x2 - x1 + 2, y2 - y1 + 2, color, color, color, 1);
        }
    }
